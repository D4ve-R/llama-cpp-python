...
from fastapi import Depends, APIRouter, Request, Response, UploadFile, HTTPException
from fastapi.responses import JSONResponse
from fastapi.routing import APIRoute
from pydantic import BaseModel, Field
from sse_starlette.sse import EventSourceResponse

from llama_cpp.server.model import get_llama, llama_outer_lock, LlamaProxy as Llama
from llama_cpp.server.settings import get_settings, ModelSettings
...
router = APIRouter(route_class=RouteErrorHandler, prefix='/v1')
...
async def upload_file(
    ...
    if not file.filename.endswith('.gguf'):
        raise HTTPException(418, "Only .gguf files can be uploaded")
    async with await open_file(settings.model, 'wb') as out_file:
        while content := await file.read(1024*1024*50): # 50MB chunks in-memory
